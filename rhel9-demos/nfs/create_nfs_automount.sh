...
ssh servera mkdir -p /new_projects/proj{1,3}
...
ssh servera 'echo "# /new_projects/proj3 172.25.250.0/24(rw,sync)" >> /etc/exports.d/project_shares.exports'
ssh servera exportfs -ra
ssh servera exportfs

# setup the nfs client to automount indirect with wildcard
dnf install -y autofs
systemctl enable autofs
echo '/my_projects    /etc/auto_projects.autofs' > /etc/auto.master.d/auto_master_projects.autofs
